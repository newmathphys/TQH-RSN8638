...
| $N = 8638 = \mathrm{round}(63/\alpha + 3\pi/2)$ | $63=\mathrm{dim}(SU(8))$, $3\pi/2$ = hemisphere volume. $8637.98\to 8638$, $\Delta=0.0002\%$ |
| $k = \gamma_1\alpha/16$ | Berry-Keating operator on the lattice |
| $\varepsilon_{\mathrm bare} = 1/14$ | Adelic structure $N = 2\cdot7\cdot617$ |
| $\varepsilon_{\mathrm eff} = 9/125$ | $\beta_0^{\mathrm QCD}/5^3$ with radiative corrections ($\Delta=\alpha_s\ln(N)/(2\pi)$) |
| $\alpha = e^{3/2}/8.5^3$ | $8.5 = \mathrm{dim}(SO(17))/\mathrm{dim}(Cl_4) = 136/16$ |
...
| $n_{\mathrm im}$ hierarchy: 15, 7.5, 2, 0.637, 0.0026 | From $\mathrm{dim}(SU(4))$, $\mathrm{dim}(G_2)/2$, $\mathrm{dim}(SU(2))$, $2/\pi$, $\varepsilon^2/\varphi$ |
...
| $\sigma_{\mathrm DM}$ (vorton 568 GeV) | $\sim 2\times10^{-47}$ cm² | Dilaton exchange calculation |
...
| Question | Comment |
|----------|---------|
| $m_e$ as absolute scale | $m_e = M_{\mathrm Pl}/\exp(k\cdot7993)$ — one free scale parameter |
...
|---------|----------|----------|
| **$m_e$ as input parameter** | $m_e = M_{\mathrm Pl} / \exp(k\cdot 7993)$, where $M_{\mathrm Pl} = \sqrt{\hbar c/G}$ | 0.08% |
| **$\Delta M_{np}$** (1.293 MeV) | $n_n - n_p = 3\varepsilon = 0.216$ ($3 = N_c$ color charges) | 1.25% |
| **$n$ for hadrons** | $n_p = 14\cdot(6\cdot\mathrm{dim}(G_2)-1) + \mathrm{dim}(Cl_4)/4 = 1166$ from $G_2\times Cl_4$; leptons from adelic factorization | ✅ |
...
| GW amplitude $\Omega_{\mathrm GW}h^2$ | Estimate $\sim 10^{-12}$, needs full calculation |
